...
class LSBMPDataset(Dataset):
    ...
    def __init__(
        ...
        for id in self.ids:
            with open(os.path.join(args.dirname, id, "empty.bin"), 'rb') as fp:
                self.empties.append(pickle.load(fp))
            with open(os.path.join(args.dirname, id, "actions.bin"), 'rb') as fp:
                self.actions.append(pickle.load(fp))
            states = []
            for i in range(self.traj_length):
                with open(os.path.join(args.dirname, id, f"state_{i}.bin"), 'rb') as fp:
    ...
    def __getitem__(self, i):
        ...
        x_t = self.state[env_index][action_index]
        x_tplus = self.state[env_index][action_index + 1]
        x_empty = self.empties[env_index]
        u_t = self.actions[env_index][action_index].copy()
